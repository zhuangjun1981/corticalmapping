...
def add_rois_and_traces(data_folder, nwb_f, mov_path):

    mov_grp = nwb_f.file_pointer[mov_path + '/corrected']
    ...
    if traces_center_raw.shape[1] != mov_grp['num_samples'].value:
        raise ValueError('number of trace time points ({}) does not match frame number of '
                         'corresponding movie ({}).'.format(traces_center_raw.shape[0], mov_grp['num_samples'].value))

    rf_img = tf.imread(os.path.join(data_folder, 'corrected_mean_projection.tif'))

    print 'adding segmentation results ...'
    rt_mo = nwb_f.create_module('rois_and_traces_plane0')
    is_if = rt_mo.create_interface('ImageSegmentation')
    is_if.create_imaging_plane('imaging_plane', description='')
    is_if.add_reference_image('imaging_plane', 'mean_projection', rf_img)
    ...
    for i in range(mask_arr_c.shape[0]):
        ...
        is_if.add_roi_mask_pixels(image_plane='imaging_plane', roi_name=curr_cen_n, desc='',
                                  pixel_list=curr_cen_pixels_xy, weights=curr_cen_roi.weights, width=resolution, height=resolution)
        ...
        is_if.add_roi_mask_pixels(image_plane='imaging_plane', roi_name=curr_sur_n, desc='',
                                  pixel_list=curr_sur_pixels_xy, weights=None, width=resolution, height=resolution)
    is_if.finalize()



    trace_f_if = rt_mo.create_interface('Fluorescence')
    seg_if_path = '/processing/rois_and_traces_plane0'+ '/ImageSegmentation/imaging_plane'
    # print seg_if_path
    ts_path = mov_path + '/corrected'
    ...
    roi_center_n_path = '/processing/rois_and_traces_plane0/Fluorescence/f_center_raw/roi_names'
    ...
    # trace_demix_ts.set_time_as_link(ts_path)
    ...
    trace_sub_ts.set_time_as_link(ts_path)
...
add_rois_and_traces(data_folder=curr_folder, nwb_f=nwb_f,
                    mov_path='/processing/motion_correction/MotionCorrection/MotionCorrection')
