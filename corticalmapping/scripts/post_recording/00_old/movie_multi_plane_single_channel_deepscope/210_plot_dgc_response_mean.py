...
response_table_path = 'analysis/response_table_001_DriftingGratingCircleRetinotopicMapping'

baseline_span = [-0.5, 0.]
response_span = [0., 1.5]
is_add_to_trace = True
...
for plane_n in plane_ns:
    ...
    pdff = PdfPages(os.path.join(save_folder, 'STA_DriftingGrating_' + plane_n + '_mean.pdf'))
    ...
    for roi_i, roi_n in enumerate(roi_lst):
        ...
        for grating_n in grating_ns:
            ...
            _ = get_dff(traces=curr_sta, t_axis=t_axis, response_span=response_span, baseline_span=baseline_span)
        ...
        for grating_n in grating_ns:
            ...
            ax.fill_between(t_axis, dff_trace_mean - dff_trace_sem, dff_trace_mean + dff_trace_sem, edgecolor='none',
