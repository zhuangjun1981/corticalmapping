...
for plane_n in plane_ns:
    ...
    if is_add_to_traces:
        add_to_trace = h5py.File("{}/caiman_segmentation_results.hdf5".format(plane_n))['bias_added_to_movie'].value
    else:
        add_to_trace = 0.
    ...
    for roi_ind, roi_n in enumerate(roi_ns):
        ...
        ax_single.set_xticks(range(len(rf_on.aziPos))[::10])
        ax_single.set_xticklabels(['{:05.1f}'.format(l) for l in rf_on.aziPos[::10]])
        ax_single.set_yticks(range(len(rf_on.altPos))[::10])
        ax_single.set_yticklabels(['{:05.1f}'.format(l) for l in rf_on.altPos[::-1][::10]])
        ax_single.set_aspect('equal')
        ax_single.set_title('{}: {}. ON thr:{}; OFF thr:{}.'.format(plane_n, roi_n, rf_on.thr, rf_off.thr))
    ...
    ax_all.set_xticks(range(len(rf_on.aziPos))[::10])
    ax_all.set_xticklabels(['{:05.1f}'.format(l) for l in rf_on.aziPos[::10]])
    ax_all.set_yticks(range(len(rf_on.altPos))[::10])
    ax_all.set_yticklabels(['{:05.1f}'.format(l) for l in rf_on.altPos[::-1][::10]])
