...
def loadTrial(trialPath):
    ...
    trial = RetinotopicMappingTrial(mouseID=trialDict['mouseID'],  # str, mouseID
                                    ...
                                    visualStimBackground=trialDict['visualStimBackground'],
                                    # str, background of visual stimulation
                                    imageExposureTime=trialDict['imageExposureTime'],
                                    # float, exposure time of image file
...
def generatePhaseMap(movie, cycles=1, isReverse=False, isFilter=False, sigma=3., isplot=False):
    ...
    if isplot == True:
        ...
        plt.imshow(plotMap, cmap='hsv', aspect='equal')
        plt.colorbar()
...
def plotPatches(patches, plotaxis=None, zoom=1, alpha=0.5, markersize=5):
    ...
    for key, value in patches.iteritems():
        ...
        h = plotaxis.imshow(currPatch.getSignedMask(), cmap='jet', vmax=1, vmin=-1, interpolation='nearest',
                            alpha=alpha)
    ...
    for ind, value in enumerate(forPlotting):
        ...
        if value[3] == -1:
            pt.plot_mask(value[2], plotAxis=plotAxis, color='#0000ff', borderWidth=borderWidth,
                         closingIteration=closingIteration)
        elif value[3] == 1:
...
def plotPairedPatches(patch1,
    ...
    patchPlot1 = f_121.imshow(patch1.getMask(), cmap='jet', interpolation='nearest', alpha=0.5, vmax=2, vmin=1)
    patchPlot2 = f_121.imshow(patch2.getMask() * 2, cmap='jet', interpolation='nearest', alpha=0.5, vmax=2, vmin=1)
    f_121.set_title('patch1: blue, patch2: red')

    f_122 = f.add_subplot(122)
    areaPlot1 = f_122.imshow(visualSpace1, cmap='jet', interpolation='nearest', alpha=0.5, vmax=2, vmin=1)
    areaPlot2 = f_122.imshow(visualSpace2 * 2, cmap='jet', interpolation='nearest', alpha=0.5, vmax=2, vmin=1)
...
class RetinotopicMappingTrial(object):
    ...
    def _getEccentricityMap(self, isPlot=False):
        ...
        if isPlot:
            plt.figure()
            plt.imshow(eccMapf, cmap='hsv', interpolation='nearest')
            plt.colorbar()
                ...
                if NumOfMin == 1:
                    ...
                    if len(newPatches) > 1:
                        ...
                        for key2, value2 in newPatches.iteritems():
                            ...
                            f121.imshow(currArray, cmap='jet', interpolation='nearest', vmin=0,
                            ...
                            f122.imshow(currVisualSpace, cmap='jet', interpolation='nearest', alpha=0.5, vmin=0,
                                        vmax=len(newPatches.keys()))
    ...
    def getTraces(self,
        ...
        for i, ROIcenter in enumerate(ROIcenters):
            try:
                altIndex = np.logical_and(altPosMapf > ROIcenter[1] - ROIsearchRanges[i],
                                          altPosMapf < ROIcenter[1] + ROIsearchRanges[i])
                aziIndex = np.logical_and(aziPosMapf > ROIcenter[0] - ROIsearchRanges[i],
                                          aziPosMapf < ROIcenter[0] + ROIsearchRanges[i])
                ...
                print 'ROI' + str(i) + ' center: [' + str(index[0][pos]) + ',' + str(
                    index[1][pos]) + ']; visual space: [' + str(altPosMapf[index[0][pos], index[1][pos]]) + ',' + str(
                    aziPosMapf[index[0][pos], index[1][pos]]) + ']'
        ...
        if isPlot:
            ...
            for i in range(len(traces)):
                try:
                    ...
                    ROIlegend.append(
                        ['center:' + str(traces[i]['position']) + ', baseline:' + '{0:.0f}'.format(np.mean(currTrace))])
                except:
                    pass
            ...
            for i in range(len(traces)):
                try:
                    ax2.plot([traces[i]['tracePhaseTime'], traces[i]['tracePhaseTime']], [ymin, ymax], '--',
                             color=traces[i]['ROIcolor'], lw=2)
                except:
                    pass
    ...
    def generateStandardOutput(self, traces=None, isSave=False, saveFolder=None):
        ...
        if traces:
            ...
            for i in range(len(traces)):
                f_3389.plot([traces[i]['tracePhaseTime'], traces[i]['tracePhaseTime']], [ymin, ymax], '--',
                            color=traces[i]['ROIcolor'], lw=2)
        ...
        currfig = f_332.imshow(np.mean([self.altPowerMap, self.aziPowerMap], axis=0), cmap='hot',
                               interpolation='nearest')
        f.colorbar(currfig)
        f_332.set_axis_off()
        ...
        if isSave:
            f.savefig(os.path.join(saveFolder, trialName + '.pdf'), format='pdf', dpi=300, orientation='landscape',
                      papertype='a4')
            f.savefig(os.path.join(saveFolder, trialName + '.png'), format='png', dpi=300, orientation='landscape',
                      papertype='a4')
            del f
    ...
    def plotTrial(self, isSave=False, saveFolder=None):
        ...
        for key, value in self.rawPatches.iteritems():
            currfig = f2_221.imshow(self.altPosMapf * value.getMask(), cmap='jet', vmin=-40, vmax=60,
                                    interpolation='nearest')
        f2.colorbar(currfig)
        plt.tick_params(
        ...
        for key, value in self.rawPatches.iteritems():
            currfig = f2_222.imshow(self.aziPosMapf * value.getMask(), cmap='jet', vmin=-10, vmax=120,
                                    interpolation='nearest')
        f2.colorbar(currfig)
        plt.tick_params(
            axis='both',  # changes apply to the x-axis
        ...
        currfig = f2_224.imshow(self.eccentricityMapf, cmap='jet', interpolation='nearest')
        plt.tick_params(
            axis='both',  # changes apply to the x-axis
            which='both',  # both major and minor ticks are affected
            bottom='off',  # ticks along the bottom edge are off
...
class Patch(object):
    ...
    def split2(self, eccMap, patchName='patch00', cutStep=1, borderWidth=2, isplot=False):
        ...
        if isplot:
            plt.figure()
            plt.subplot(121)
            plt.imshow(self.array, cmap='jet', interpolation='nearest')
            plt.title(patchName + ': before split')
            plt.subplot(122)
            plt.imshow(labeledNewPatchMap, cmap='jet', interpolation='nearest')
    ...
    def split(self, eccMap, patchName='patch00', cutStep=1, borderWidth=2, isplot=False):
        ...
        plt.figure()
        plt.imshow(minMarker, cmap='jet', vmin=0, interpolation='nearest')
        plt.colorbar()
        plt.title('markers 1')
        plt.show()
        ...
        plt.figure()
        plt.imshow(newLabel, cmap='jet', vmin=0, interpolation='nearest')
        plt.colorbar()
        plt.title('markers 3')
        plt.show()
...
if __name__ == "__main__":
    ...
    # testTrial, traces = loadTrial(r"E:\data2\2015-02-03-population-maps\populationTial_Ai93&Ai9630min.pkl")
    testTrial, traces = loadTrial(
        r"E:\data\2015-11-13-150821-M177931-RetinotopicMapping\20150821_MM177931_Trial1_3_4.pkl")
