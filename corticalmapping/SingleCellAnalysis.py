...
from . import core.PlottingTools as pt
...
class SpatialReceptiveField(WeightedROI):
    ...
    def plot_rf(self, plot_axis=None, is_colorbar=False, cmap='Reds', interpolation='nearest', **kwargs):
        ...
        plot_axis.set_yticks(list(range(len(self.altPos)))[::interpolate_rate])
        plot_axis.set_xticks(list(range(len(self.aziPos)))[::interpolate_rate])
    ...
    def plot_contour(self, plot_axis=None, peak_amplitude=None, level_num=10, **kwargs):
        ...
        if self.interpolate_rate is not None:
            plot_axis.set_yticks(list(range(len(self.altPos)))[::self.interpolate_rate])
            plot_axis.set_xticks(list(range(len(self.aziPos)))[::self.interpolate_rate])
            plot_axis.set_yticklabels(self.altPos[::self.interpolate_rate])
            plot_axis.set_xticklabels(self.aziPos[::self.interpolate_rate])
        else:
            plot_axis.set_yticks(list(range(len(self.altPos))))
            plot_axis.set_xticks(list(range(len(self.aziPos))))
